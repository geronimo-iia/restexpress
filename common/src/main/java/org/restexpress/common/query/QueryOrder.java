...
public class QueryOrder {
	 ...
	 *            descending).
	 */
	public QueryOrder(final String... strings) {
		this();
	 ...
	 */
	public QueryOrder addSort(final String... strings) {
		if (sorts == null) {
			sorts = new ArrayList<OrderComponent>(strings.length);
		}

		for (final String sortString : strings) {
			final boolean isDescending = sortString.startsWith("-");
			final String fieldName = sortString.replaceAll("^[+-]{1}", "");
	...
	public void iterate(final OrderCallback callback) {
		if ((callback == null) || !isSorted()) {
			return;
		}

		for (final OrderComponent component : sorts) {
