...
import org.restexpress.serialization.xml.Aliasable;

/**
 * ResponseProcessorManager manager {@link ResponseProcessor}. This class
 * implements a {@link SerializationProvider} and a
...
public class ResponseProcessorManager implements ResponseProcessorSettingResolver, SerializationProvider {
	...
	public void alias(String name, Class<?> theClass) {
		for (ResponseProcessor responseProcessor : processorsByMediaType.values()) {
			if (Aliasable.class.isAssignableFrom(responseProcessor.processor().getClass())) {
				((Aliasable) responseProcessor.processor()).alias(name, theClass);
			}
		}
	}

	/**
	 * Add specified {@link MediaRange} if they're not ever exists. TODO add
	 * test case
	 * 
	 * @param mediaRanges
	 */
