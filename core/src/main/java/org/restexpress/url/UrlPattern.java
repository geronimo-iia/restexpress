 ...
 * <p/>
 * An optional format parameter following a dot ('.') may be added to the end.
 * While it could be named any valid parameter name, RestExpress offers special
 * handling (e.g. within the Request, etc.) if it's named 'format'.
 * <p/>
 * Note that the format specifier allows only word characters and
 ...
 * </ul>
 * 
 * RestExpress parses URI paths which is described in the URI Generic Syntax
 * IETF RFC 3986 specifcation, section 3.3
 * (http://tools.ietf.org/html/rfc3986#section-3.3). RestExpress parses paths
 * into segments separated by slashes ("/"), the segments of which are composed
 * of unreserved, percent encoded, sub-delimiters, colon (":") or asperand
 * ("@"), each of which are defined below (from the spec):
 * <p/>
 * pct-encoded = "%" HEXDIG HEXDIG
 * <p/>
 * unreserved = ALPHA / DIGIT / "-" / "." / "_" / "~"<br/>
 * reserved = gen-delims / sub-delims<br/>
 * gen-delims = ":" / "/" / "?" / "#" / "[" / "]" / "@"</br> sub-delims = "!" /
 * "$" / "&" / "'" / "(" / ")" / "*" / "+" / "," / ";" / "=" *
 * <p/>
 * In other words, RestExpress accepts path segments containing: [A-Z] [a-z]
 * [0-9] % - . _ ~ ! $ & ' ( ) * + , ; = : @
 * <p/>
 * RestExpress also accepts square brackets ('[' and ']'), but this is
...
public class UrlPattern implements UrlMatcher {
	...
	// Replaces parameter names in the URL pattern string to match parameters in
	// URLs.
	...
	// Replaces the format parameter name in the URL pattern string to match the
	// format specifier in URLs. Appended to the end of the regex string
	// when a URL pattern contains a format parameter.
	private static final String URL_FORMAT_MATCH_REGEX = "(?:\\\\.\\([\\\\w%]+?\\))?";

	// Finds the query string portion within a URL. Appended to the end of the
	// built-up regex string.
	 ...
	 * @param pattern
	 */
	public UrlPattern(final String pattern) {
		super();
	 ...
	 *            the pattern to set
	 */
	private void setUrlPattern(final String pattern) {
	...
	/**
	 * Test the given URL against the underlying pattern to determine if it
	 * matches, returning a boolean to reflect the outcome.
	 * 
	 * @param url
	 *            an URL string with or without query string.
	 * @return true if the given URL matches the underlying pattern. Otherwise
	 *         false.
	 */
	@Override
	...
	/**
	 * Processes the incoming URL pattern string to create a java.util.regex
	 * Pattern out of it and parse out the parameter names, if applicable.
	 */
	public void compile() {
	...
	/**
	 * Parses the parameter names from the URL pattern string provided to the
	 * constructor, building the ordered list, parameterNames.
	 */
	private void acquireParameterNames() {
		final Matcher m = URL_PARAM_PATTERN.matcher(getUrlPattern());
	...
	/**
	 * Extracts parameter values from a Matcher instance using the regular
