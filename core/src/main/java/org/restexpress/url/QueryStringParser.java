 ...
 * parameter.
 * <p/>
 * Note that no exception is thrown if the number of parameters is exceeded.
 * This process simply stops adding parameters over the limit.
 * 
 * <p>
...
public class QueryStringParser {
	 ...
	 */
	public QueryStringParser(final String uri) {
		this(uri, true);
	}
	 ...
	 * specified charset.
	 */
	public QueryStringParser(final String uri, final boolean hasPath) {
		this(uri, hasPath, DEFAULT_MAX_PARAMS);
	}

	public QueryStringParser(final URI uri) {
	 ...
	 * specified charset.
	 */
	public QueryStringParser(final String uri, final boolean hasPath, final int maxParams) {
		if (uri == null) {
	 ...
	 * specified charset.
	 */
	public QueryStringParser(final URI uri, final int maxParams) {
		if (uri == null) {
			throw new NullPointerException("uri");
		}
		if (maxParams <= 0) {
		...
		final String newUri = rawPath + "?" + uri.getRawQuery();
	...
	public Map<String, List<String>> getParameters() {
		if (params == null) {
			if (hasPath) {
				final int pathLength = getPath().length();
	...
	private void parseParams(final String s) {
		final Map<String, List<String>> params = this.params = new LinkedHashMap<String, List<String>>();
		nParams = 0;
		String name = null;
