 ...
 * 
 * <p/>
 * Request have parameters from route definition and attachment from runtime.
 * Both of them concern are pair of {key, value}. <br />
 ...
 * what permission is being requested on a given resource. <br>
 * <br>
 * Each request can have many user-defined attachments, perhaps placed via
 * preprocessors, etc. These attachments are named and are carried along with
 * the request to subsequent preprocessors, controllers, and postprocessors.
 * Attachments are different than parameters in that, they are set on a per
 * request basis, instead of at the route level. They can be set via
 * preprocessors, controllers, postprocessor, as opposed to parameters which are
 * set on the route definition.
 * <p/>
 * The Correlation ID for each request is unique within this VM instance.
 * Restarting the VM will reset the correlation ID to zero. It is not a GUID. It
 * is useful, however, in correlating events in the pipeline (e.g. timing,
 * etc.).
 * 
 * @author toddf
 * @since Nov 20, 2009
