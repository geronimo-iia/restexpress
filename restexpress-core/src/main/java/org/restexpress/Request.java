...
/**
 * {@link Request} object definition.
 * 
 * <p/>
 * Request have parameters from route definition and attachment from runtime. Both of them concern are pair of {key, value}. <br />
 ...
 * preprocessors, controllers, or postprocessors. This is a way to pass data from a route definition down to subsequent controllers,
 * etc. An example might be: setParameter("route", "read_foo") setParameter("permission", "view_private_data"), which might inform an
 * authorization preprocessor of what permission is being requested on a given resource. <br>
 * <br>
 * Each request can have many user-defined attachments, perhaps placed via preprocessors, etc. These attachments are named and are
 * carried along with the request to subsequent preprocessors, controllers, and postprocessors. Attachments are different than
 * parameters in that, they are set on a per request basis, instead of at the route level. They can be set via preprocessors,
 * controllers, postprocessor, as opposed to parameters which are set on the route definition.
 * <p/>
 * The Correlation ID for each request is unique within this VM instance. Restarting the VM will reset the correlation ID to zero. It
 * is not a GUID. It is useful, however, in correlating events in the pipeline (e.g. timing, etc.).
 * 
 * @author toddf
 * @since Nov 20, 2009
 */
public class Request {
    ...
    public Request(final HttpRequest httpRequest, final RouteResolver routeResolver,
            final ResponseProcessorSettingResolver responseProcessorSettingResolver) {
     ...
     * @param routeResolver {@link RouteResolver}.
     * @param responseProcessorSettingResolver {@link ResponseProcessorSettingResolver}.
    ...
    /**
     * Return the Correlation ID for this request. The Correlation ID is unique for each request within this VM instance. Restarting
     * the VM will reset the correlation ID to zero. It is not a GUID. It is useful, however, in correlating events in the pipeline
     * (e.g. timing, etc.).
     */
    ...
    /**
     * Return the requested HTTP method of the request, whether via the request's HTTP method or via a query parameter (e.g.
     * "_Method=").
     * 
    ...
    /**
     * Attempts to deserialize the request body into an instance of the given type. If the serialization process returns null, throws
    ...
    /**
     * Accesses the request body as a ByteBuffer. It is equivalent to calling getBody().toByteBuffer().
     * 
     * @return a ByteBuffer.
     */
    ...
    /**
     * Returns the byte array underlying the Netty ChannelBuffer for this request. However, if the ChannelBuffer returns false to
     * hasArray(), this method returns null.
     * 
    ...
    /**
     * Returns the body as a Map of name/value pairs from a url-form-encoded form submission. The values returned are URL-decoded.
     * 
     * @return
     */
    ...
    /**
     * Returns the body as a Map of name/value pairs from a url-form-encoded form submission. The values returned are URL-decoded
     * depending on the value of shouldDecode.
     * 
    ...
    /**
     * Gets the named header from the request. Returns null if the header is not present. Both HTTP headers and query-string parameters
     * are set as headers on the Request, with query-string parameters overriding headers if there is a name clash.
     * <p/>
     * NOTE: because HTTP headers are handled by Netty, which processes them with QueryStringDecoder, HTTP headers are URL decoded.
     * Also query-string parameters that get processed by RestExpress are URL decoded before being set as headers on the request.
     * 
     * @param name
    ...
    /**
     * Gets the list of named headers from the request. Returns null if the header name is not present.
     * <p/>
     * NOTE: because HTTP headers are handled by Netty, which processes them with QueryStringDecoder, HTTP headers are URL decoded.
     * Also query-string parameters that get processed by RestExpress are URL decoded before being set as headers on the request.
     * 
    ...
    /**
     * Gets the named header from the request. Throws BadRequestException(message) if the header is not present. Both HTTP headers and
     * query-string parameters are set as headers on the Request, with query-string parameters overriding headers if there is a name
     * clash.
     * <p/>
     * NOTE: because HTTP headers are handled by Netty, which processes them with QueryStringDecoder, HTTP headers are URL decoded.
     * Also query-string parameters that get processed by RestExpress are URL decoded before being set as headers on the request.
     ...
     * 
     * Beware that headers set when parsing the HTTP request are indistinguishable from those set programmatically via this method.
     * Consider using {@link #putAttachment} to persist data that should not be directly accessible by the request.
     */
    ...
    /**
     * Returns the full URL for the request, containing protocol, host and path. Note that this call also returns the query string as
     * part of the path.
     * 
    ...
    /**
     * @return {@link Boolean#TRUE} if and only if the connection can remain open and thus 'kept alive'.
     */
    ...
    /**
     * @return {@link Boolean#TRUE} if and only if this message does not have any content
     */
    ...
    /**
     * Checks the format request parameter against the given format value. Ignores case.
     * 
     * @param format
    ...
    /**
     * Checks the value of the given header against the given value. Ignores case. If the header value or given value is null or has a
     * trimmed length of zero, returns false.
     * 
    ...
    /**
     * Ask if the request contains the named flag. Flags are boolean settings that are created at route definition time. These flags
     * can be used to pass booleans to preprocessors, controllers, or postprocessors. An example might be: flag(NO_AUTHORIZATION),
     * which might inform an authorization preprocessor to skip authorization for this route.
     * 
    ...
    /**
     * Get a named parameter. Parameters are named settings that are created at route definition time. These parameters can be used to
     * pass data to subsequent preprocessors, controllers, or postprocessors. This is a way to pass data from a route definition down
     * to subsequent controllers, etc. An example might be: setParameter("route", "read_foo") setParameter("permission",
     * "view_private_data"), which might inform an authorization preprocessor of what permission is being requested on a given
     * resource.
     * 
    ...
    /**
     * Set an attachment on this request. These attachments are named and are carried along with the request to subsequent
     * preprocessors, controllers, and postprocessors. Attachments are different than parameters in that, they are set on a per request
     * basis, instead of at the route level. They can be set via preprocessors, controllers, postprocessor, as opposed to parameters
     * which are set on the route definition.
     * 
    ...
    /**
     * Add the query string parameters to the request as headers. Also parses the query string into the queryStringMap, if applicable.
     * Note, if the query string contains multiple of the same parameter name, the headers will contain them all, but the
    ...
    /**
     * If the request HTTP method is post, allow a query string parameter to determine the request HTTP method of the post (e.g.
     * _method=DELETE or _method=PUT). This supports DELETE and PUT from the browser.
     * 
     * @param parameters
