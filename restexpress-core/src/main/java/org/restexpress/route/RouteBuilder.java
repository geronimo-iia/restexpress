 ...
 * Change
 * <ul>
 * <li>When using default mapping only, this builder did not raise an exception
 * if a declared action has no method, but log a warning.</li>
...
public abstract class RouteBuilder {
	 ...
	 * @return
	 */
	protected abstract Route newRoute(String pattern, Invoker invoker, HttpMethod method, boolean shouldSerializeResponse, String name, Set<String> flags, Map<String, Object> parameters);
	...
	/**
	 * Map a service method (action) to a particular HTTP method (e.g. GET,
	 * POST, PUT, DELETE, HEAD, OPTIONS)
	 * 
	 * @param invoker
	 *            {@link Invoker} instance to call the method within the service
	 *            POJO.
	 * @param method
	 *            the HTTP method that should invoke the service method.
	 * @return the RouteBuilder instance.
	 ...
	 * @return a {@link RouteDefinition} instance
	 */
	public RouteDefinition build(RestExpress restExpress) {
		if (methods.isEmpty()) {
		...
		for (final HttpMethod httpMethod : methods) {
			...
			if (invoker == null) {
				...
				if (actionName == null) {
					// on default mapping, if method is not found we did not
					// raise a configuration exception. We just log a warning.
				...
				if (action != null) {
					...
					// find a good invoker
					invoker = Invokers.newInvoker(restExpress.paramConverterProvider(), controller, action);
				}
		...
		// build meta data and return a RouteDefinition.
		return new RouteDefinition(routes, asMetadata(routes));
	}

	/**
	 * Should Serialize Response for specified action ? This method return false
	 * if action return type is
	 * <ul>
	 * <li>a {@link File}</li>
	 * <li>a {@link ChannelBuffer}</li>
	 * <li>shouldSerializeResponse flag is false (from call on
	 * {@link #noSerialization()})</li>
	 * </ul>
	 * 
	 * @param action
	 *            {@link Method} to check
	 ...
	 */
	private RouteMetadata asMetadata(List<Route> routes) {
